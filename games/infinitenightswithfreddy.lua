...
MainSection:NewToggle("Always Green/Win Gamble", "teleports ball into green and win gamble", function(state)
	getgenv().winninggambletoggle = state
	while getgenv().winninggambletoggle do
			game:GetService("Workspace").Shop.FunMachine.Balls:WaitForChild("BaseBall").CFrame = game:GetService("Workspace").Shop.FunMachine.Zones.GOOD.CFrame
			task.wait()
		if not getgenv().winninggambletoggle then
			break
		end
	end
end)
