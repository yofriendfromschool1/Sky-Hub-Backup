...
MainSection:NewToggle("guess the word", "guesses all possible words by looking at amount of letters and which are revealed", function(state)
	...
	while true do 
		...
		for i, v in pairs(words) do
			if matchPattern(game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui"):FindFirstChild("TheWord"):FindFirstChild("Frame"):FindFirstChild("word").Text, v) then
				local args = {
					[1] = v
				}
...
MainSection:NewToggle("block drawer cam", "blocks the person drawing camera", function(state)
	...
	if state then
		...
		repeat
			...
			game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = CFrame.new(-2, 26, -67)
			task.wait()
